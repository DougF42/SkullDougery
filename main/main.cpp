...
void play_task(void *param)
{
	...
	xTaskCreatePinnedToCore( SwitchBoard::runDelivery, "SwitchBoard",
			8192, nullptr, 2, &switchboardHandle, ASSIGN_SWITCHBOARD_CORE);
  ...
  while (true)
  {
    ...
    if (!fp)
    {
      ESP_LOGE("main", "Failed to open file. Error %d (%s)", errno, strerror(errno));
      continue;
    }
    while (1)
    {
